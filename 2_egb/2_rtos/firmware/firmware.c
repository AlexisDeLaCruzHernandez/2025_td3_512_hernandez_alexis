...
#define UART_TX_PIN 12                  // Pin TX de UART
#define UART_RX_PIN 13                  // Pin RX de UART
...
typedef enum {                          // Distintas variables accesibles por UART
    ...
    VAR_PKD,                            // Variable kd
    VAR_PKI,                            // Variable ki
    VAR_VOB,                            // Variable velocidad_objetivo
    VAR_VME,                            // Variable velocidad_medida
    VAR_TAC,                            // Variable t_aceleracion
    VAR_TDE,                            // Variable t_desaceleracion
    VAR_PWM,                            // Ciclo de actividad
    VAR_MEM,                            // SD conectada
    VAR_SON,                            // Inicio de control
    VAR_TIM                             // Variable de tiempo
} cmd_variable_t;
...
QueueHandle_t cola_uart;                // Cola donde se mandan los caracteres recibidos por UART
QueueHandle_t cola_sd;                  // Cola que indica el estado de la sd
QueueHandle_t cola_inicio;              // Cola para saber si ya termino la inicializacion
...
/**
 * @brief Interrupcion de UART, envia los caracteres recibidos
 */
void uart_irq_handler() {
    char c;
    BaseType_t to_higher_priority_task = pdFALSE;
    c = uart_getc(UART_ID);
    xQueueSendToBackFromISR(cola_uart, &c, &to_higher_priority_task);
    portYIELD_FROM_ISR(to_higher_priority_task);
}
...
void task_datalogger(void *params) {
    ...
    uint8_t mensaje_sd, datos = 0, estado_sd = 0;
    uint16_t i = 0;
...
void task_seteo(void *params) {
    ...
    bool cambiar_hora = 0;
    char fecha[9];
    uint8_t unidad = 0, inicio = 0;
    // Indicamos que no se inicio
    ...
    while(1) {
        ...
        xQueuePeek(cola_inicio, &inicio, portMAX_DELAY);

        if(inicio == 1) { // En caso de que inicio este en uno significa que se configuro por UART
            encoder.evento = EVENTO_CALIBRAR; // Simulamos evento del boton calibrar
            encoder.sw = 1; // Simulamos que terminamos de configurar la hora
            cambiar_hora = 0; // Indicamos que no hubo cambio de hora para no superponer lo enviado por UART
        }

        if(encoder.evento == EVENTO_CALIBRAR) { 
            if(encoder.sw == 0) { // Si se pulso activamos la configuracion y cambiamos entre valores
                cambiar_hora = 1;
                unidad++;
...
void task_uart(void *params) {
    ...
    const var_map_t var_map[] = { // Tabla con las variables a verificar
        {"pkp", VAR_PKP},
        {"pkd", VAR_PKD},
        {"pki", VAR_PKI},
        {"vob", VAR_VOB},
    ...
    uint8_t i = 0, j, recepcion = 0, error = 0, inicio = 0;
    cmd_tipo_t tipo_i;
    ...
    while(1) {
        ...
        // Verificamos si se recibio basura
        if(in_buff[0] != 'g' && in_buff[0] != 's') i = 0;
        ...
        if(recepcion == 1) {
            ...
            tipo = strtok(in_buff, " "); // Obtenemos un string con el tipo de comando
            variable = strtok(NULL, " "); // Obtenemos un string con el tipo de variable
            dato = strtok(NULL, " "); // Obtenemos un string con el dato a guardar
            ...
            if(!error) {
                ...
                if(j == 11) error = 3; // Error de variable desconocida
            }
            // Procesamiento
            if(!error) {
                switch(inicio) {
                    case 0:
                        if(tipo_i == CMD_SET && variable_i == VAR_TIM) {
                            sscanf(dato, "%hhd:%hhd:%hhd-%hhd/%hhd/%hhd", 
                                &fecha_hora.hour,
                                &fecha_hora.minute,
                                &fecha_hora.second,
                                &fecha_hora.date,
                    ...
                    case 2:
                        ...
                        if(tipo_i == CMD_GET) {
                            ...
                            switch(variable_i) {
                                case VAR_PKP:
                                    sscanf(dato, "%f", &pid_params.kp);
                                    xQueueOverwrite(cola_pid, &pid_params);
                                    break;
                                case VAR_PKD:
                                    sscanf(dato, "%f", &pid_params.kd);
                                    xQueueOverwrite(cola_pid, &pid_params);
                                    break;
                                case VAR_PKI:
                                    sscanf(dato, "%f", &pid_params.ki);
                                    xQueueOverwrite(cola_pid, &pid_params);
                                    break;
                                case VAR_VOB:
                                    sscanf(dato, "%d", &config_uart.velocidad_objetivo);
                                    xQueueOverwrite(cola_configuracion, &config_uart);
                                    break;
                                case VAR_TAC:
                                    sscanf(dato, "%f", &config_uart.t_aceleracion);
                                    xQueueOverwrite(cola_configuracion, &config_uart);
                                    break;
                                case VAR_TDE:
                                    sscanf(dato, "%f", &config_uart.t_desaceleracion);
                                    xQueueOverwrite(cola_configuracion, &config_uart);
                                    break;
                                case VAR_SON:
                                    xSemaphoreGive(semaforo_pulsador);
